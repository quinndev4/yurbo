...
export default function UserDataProvider({
  ...
  const [friends, setFriends] = useState<Map<string, Friend>>(Map());

  useEffect(() => {
    (async () => {
      try {
        if (session?.user?.id) {
          const [yurbos, events, locations]: [Yurbo[], Event[], Location[]] =
            await Promise.all([
          ...
          const friendMap = Map(friends.map((friend) => [friend.id, friend]));
